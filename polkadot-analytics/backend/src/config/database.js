...
const connectDB = async () => {
  ...
  try {
    ...
    // Ensure the blocks.hash column has the correct character set and collation
    try {
      // Drop existing index if it exists
      await sequelize.query(`DROP INDEX IF EXISTS uq_blocks_hash ON blocks;`);
    ...
    // Let Sequelize handle the foreign key constraints defined in the models
    console.log('ℹ️  Foreign key constraints are defined in the models and will be handled by Sequelize');

    // Ensure the block_hash column in transactions has the correct definition
    try {
      ...
      // Then create the index in a separate query
      try {
        await sequelize.query(`
          CREATE INDEX idx_transactions_block_hash ON transactions(block_hash);
        `);
        console.log('✅ Created index on transactions.block_hash');
      } catch (indexError) {
        // Ignore error if index already exists
        if (!indexError.message.includes('already exists')) {
          console.error('❌ Error creating index on transactions.block_hash:', indexError.message);
        }
      }
      
      console.log('✅ Ensured transactions.block_hash has correct definition and index');
    } catch (error) {
      console.error('❌ Error updating transactions.block_hash:', error.message);
      if (error.sql) console.error('SQL:', error.sql);
    }
