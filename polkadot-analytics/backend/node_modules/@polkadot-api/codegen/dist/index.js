...
__export(src_exports, {
  ...
  generateDescriptors: () => generateDescriptors,
  generateMultipleDescriptors: () => generateMultipleDescriptors,
  getNewTypes: () => getNewTypes,
  getTypesBuilder: () => getTypesBuilder,
  knownTypesRepository: () => knownTypesRepository,
...
var _buildSyntax = (input, cache, stack, declarations, getChecksum, knownTypes2) => {
  ...
  const buildStruct = (id, value) => {
    ...
    declarations.variables.set(id, variable2);
    const deps = (0, import_utils.mapObject)(value, buildNextSyntax);
...
var getNewTypes = (metadata, knownTypes2, getTypeName) => {
  const checksumBuilder = (0, import_metadata_builders2.getChecksumBuilder)(metadata);
  let declarations = defaultDeclarations();
  let typesBuilder = getTypesBuilder(declarations, metadata, knownTypes2);
  const lookup = (0, import_metadata_builders2.getLookupFn)(metadata.lookup);
...
var import_metadata_builders3 = require("@polkadot-api/metadata-builders");
...
var customStringifyObject = (input) => {
  ...
  return `{${Object.entries(
    (0, import_utils2.mapObject)(input, (x) => isDocs(x) ? x : customStringifyObject(x))
  ).map(([key, value]) => {
    if (isDocs(value)) {
      return `
...
var generateDescriptors = (metadata, checksums, typesBuilder, checksumBuilder, prefix, paths) => {
  ...
  const mapDescriptor = (descriptor, mapFn) => (0, import_utils2.filterObject)(
    (0, import_utils2.mapObject)(
      descriptor,
      (v, pallet) => (0, import_utils2.mapObject)(v, (value, name) => mapFn(value, pallet, name))
    ),
  ...
  const mapObjStr = import_utils2.mapObject;
  ...
  const iRuntimeCalls = (0, import_utils2.mapObject)(runtimeCalls, (api) => ({
    docs: api.docs,
    value: (0, import_utils2.mapObject)(api.methods, ({ docs, type: value }) => ({ docs, value }))
  }));
  descriptorValues["apis"] = (0, import_utils2.mapObject)(
    runtimeCalls,
    (api) => (0, import_utils2.mapObject)(api.methods, (x) => x.checksum)
  );
  const clientImports = [
    "StorageDescriptor",
    "PlainDescriptor",
  ...
  let _assetId = null;
  if (assetPayment) {
    const assetTxPayment = (0, import_metadata_builders3.getLookupFn)(metadata.lookup)(assetPayment.type);
...
var import_metadata_builders5 = require("@polkadot-api/metadata-builders");
...
var import_metadata_builders4 = require("@polkadot-api/metadata-builders");
var getUsedChecksums = (metadata, builder = (0, import_metadata_builders4.getChecksumBuilder)(metadata)) => {
...
var knownTypes = (0, import_utils3.mapObject)(
...
var generateMultipleDescriptors = (chains, paths, options = {}) => {
  const chainData = chains.map((chain) => {
    ...
    const builder = (0, import_metadata_builders5.getChecksumBuilder)(metadata);
    return {
      ...chain,
      metadata,
      builder,
...
function generateDescriptorValuesContent(descriptorValues) {
  ...
  const minified = (0, import_utils4.mapObject)(descriptorValues, minifyKeys);
  ...
  const findCommonTrees = (values) => {
    ...
    const keys = values.map(
      (obj) => (0, import_utils4.mapObject)(obj, (tree) => {
